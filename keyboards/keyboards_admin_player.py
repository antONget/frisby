...
def keyboard_edit_list_user() -> None:
    ...
    button_3 = InlineKeyboardButton(text='Редактировать',
...
def keyboards_del_users(list_users: list, back: int, forward: int, count: int):
    ...
    max_forward = whole + 1
    print("keyboards_change_users", count_users, whole, remains, max_forward)
...
def keyboards_change_users(list_users: list, back: int, forward: int, count: int):
    """
    ИГРОК -> Редактировать имя пользователя
    :param list_users: список пользователей
    :param back:
    :param forward:
    :param count:
    :return:
    """
    logging.info("keyboards_change_users")
    # считаем сколько всего блоков по заданному количество элементов в блоке
    count_users = len(list_users)
    whole = count_users // count
    remains = count_users % count
    ...
    for row in list_users[back*count:(forward-1)*count]:
        print(row)
        text = row[1]
        button = f'changeuser_{row[0]}'
        buttons.append(InlineKeyboardButton(
            text=text,
            callback_data=button))
    button_back = InlineKeyboardButton(text='<<<<',
                                       callback_data=f'changeback_{str(back)}')
    button_count = InlineKeyboardButton(text=f'{back+1}',
                                        callback_data='none')
    button_next = InlineKeyboardButton(text='>>>>',
                                       callback_data=f'changeforward_{str(forward)}')
    kb_builder.row(*buttons, width=1)
    kb_builder.row(button_back, button_count, button_next)
    return kb_builder.as_markup()


def keyboard_change_user() -> None:
    """
    МЕНЕДЖЕР -> Удалить -> подтверждение удаления менеджера из базы
    :return:
    """
    ...
    button_1 = InlineKeyboardButton(text='Редактировать',
                                    callback_data='change_name_user')
    button_2 = InlineKeyboardButton(text='Отмена',
                                    callback_data='notchange_user')
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[[button_1, button_2]],
    )
    return keyboard